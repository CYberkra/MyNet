#!/usr/bin/env python3
"""Audit a completed native-256 short full/control/air B-scan pilot."""
...
def main() -> int:
    ...
    paths = {
        ...
        "air": case_dir / "air_reference_merged.out",
    }
    ...
    time_ns, full = resample_time_axis(raw_values[0], dt_values[0], time_window_ns=700.0, output_samples=501)
    _, control = resample_time_axis(raw_values[1], dt_values[1], time_window_ns=700.0, output_samples=501)
    _, air = resample_time_axis(raw_values[2], dt_values[2], time_window_ns=700.0, output_samples=501)
    ...
    result = {
        ...
        "air_direct_rms_to_full": _rms(air[time_ns <= 150]) / max(_rms(full[time_ns <= 150]), 1e-30),
    }
    ...
    full_s = full - np.median(full, axis=1, keepdims=True)
    control_s = control - np.median(control, axis=1, keepdims=True)
    common_scale = float(np.percentile(np.abs(np.concatenate((full_s[rows], control_s[rows]), axis=1)), 99.5)) or 1.0
    ...
    for values, box, title, scale in (
        (full_s, boxes[0], "Full scene", common_scale),
        (control_s, boxes[1], "No-basal control (shared scale)", common_scale),
